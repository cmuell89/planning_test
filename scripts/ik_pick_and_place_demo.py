...
def load_gazebo_models(table_pose=Pose(position=Point(x=0.75, y=0.0, z=0.0)),
                       ...
                       block_pose=Pose(position=Point(x=0.4225, y=0.1265, z=0.7728)),
                       block_reference_frame="world",
                       green_bowl_pose=Pose(position=Point(x=0.3225, y=0.1065, z=0.7728)),
                       green_bowl_reference_frame="world"):
    # Get Models' Path
    ...
    green_bowl_xml = ''
    with open (model_path + "green_bowl/model.sdf", "r") as green_bowl_file:
        green_bowl_xml=green_bowl_file.read().replace('\n', '')
    
    # Load Red Block URDF
    red_block_xml = ''
    with open (model_path + "red_block/model.urdf", "r") as red_block_file:
        red_block_xml=red_block_file.read().replace('\n', '')
    
    # Load Green Block URDF
    green_block_xml = ''
    with open (model_path + "green_block/model.urdf", "r") as green_block_file:
        green_block_xml=green_block_file.read().replace('\n', '')
    ...
    # Spawn Green Bowl SDF
    rospy.wait_for_service('/gazebo/spawn_sdf_model')
    try:
        spawn_sdf = rospy.ServiceProxy('/gazebo/spawn_sdf_model', SpawnModel)
        resp_sdf = spawn_sdf("green_bowl", green_bowl_xml, "/",
                             green_bowl_pose, green_bowl_reference_frame)    
    except rospy.ServiceException, e:
        rospy.logerr("Spawn SDF service call failed: {0}".format(e))
    ...
    try:
        ...
        resp_urdf = spawn_urdf("red_block", red_block_xml, "/",
                               block_pose, block_reference_frame)
    except rospy.ServiceException, e:
        rospy.logerr("Spawn URDF service call failed: {0}".format(e))

    # Spawn Green Block URDF
    rospy.wait_for_service('/gazebo/spawn_urdf_model')
    try:
        spawn_urdf = rospy.ServiceProxy('/gazebo/spawn_urdf_model', SpawnModel)
        resp_urdf = spawn_urdf("green_block", green_block_xml, "/",
                               block_pose, block_reference_frame)
    except rospy.ServiceException, e:
        rospy.logerr("Spawn URDF service call failed: {0}".format(e))
